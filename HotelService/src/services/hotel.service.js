...
const createHotel = async (hotelData) => {
  if (!hotelData.name) throw new Error("Hotel name is required");
...
const updateRoomAvailability = async (hotelId, roomId, isAvailable) => {
  // Look up the room to get hotelId for cache invalidation
  const room = await hotelRepository.getRoomByIdOnly(roomId);
  if (!room) throw new Error("Room not found");

  await hotelRepository.updateRoomAvailability(hotelId, roomId, isAvailable);

  // Clear the rooms cache for this hotel so next read hits DB
  await hotelCache.clearRoomsCache(room.hotel_id);

  console.log(
    `Room ${roomId} marked ${isAvailable ? "available" : "unavailable"} in DB`,
  );
};
